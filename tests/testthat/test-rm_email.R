...
test_that("rm_email is removing/replacing email strings",{
    ...
    expect_true(identical(rm_email(x), x2))
    expect_true(identical(rm_email(x, replacement = '<a href="mailto:\\1" target="_blank">\\1</a>'), x3))	
...
test_that("rm_email is extracting email strings",{
    ...
    expect_true(identical(rm_email(x, extract=TRUE), x4))
})
